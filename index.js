const reservas = [
  ...
  console.log("Al ser un PARTICULAR el Subtotal asciende a ",precio1.Subtotal,"€ y el Toatal con IVA del 21% asciende a ",precio1.Total,"€" )

  const precio2 = new precioTour(reservas);
  console.log("Al ser un OPERADOR el Subtotal asciende a ",precio2.Subtotal,"€ y el Toatal con IVA del 21% y un descuento del 10% asciende a ",precio2.Total,"€" )
...
class bass {
  ...
  calcularSubtotal(pr1,pr2){
    this.subtotal = this.reserva.reduce((acc,reser) => acc += (((reser.tipoHabitacion ==="standard")?(pr1*reser.noches+((reser.pax-1)*40)*reser.noches):pr2*reser.noches+((reser.pax-1)*40)*reser.noches) + ((reser.desayuno)? reser.noches*15:0)) ,0 )  
  }
  calcularTotal(ds){
...
class child2 extends bass{
 ...
 precioch2.reservain = reservas;
  console.log("Al ser un PARTICULAR el Subtotal asciende a ",precioch1.subtotal,"€ y el Toatal con IVA del 21% asciende a ",precioch1.total.toFixed(2),"€" )
  console.log("Al ser un OPERADOR el Subtotal asciende a ",precioch2.subtotal,"€ y el Toatal con IVA del 21% y un descuento del 10% asciende a ",precioch2.total.toFixed(2),"€" )
